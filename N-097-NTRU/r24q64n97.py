...
global n,N,Nmax,q,p
n=N=97#19,37,59,211,107,311,413
Nmax=n+1
q=64
p=3
...
def Inverse_Poly_Fq(a,Fq,q):#when q is odd 
    k=v=0
    Poly_init(Fq,Nmax)
...
def RandPoly( r, n , NumOnes, NumNegOnes):
   ...
   while(NumOnes!=0 or NumNegOnes!=0):
      position=random321.randint(0,n-1)#including 0 and n-1
      if r[position] == 0:
         if NumOnes > 0:
            r[position] = 1
            NumOnes = NumOnes-1
...
def CreateKey(aa, bb, cc, dd, ee, ff, gg, hh):
   ...
   if(panduan_f_inv_pq(dd,p,q)!=1 ):
      if(n==107):
         ...
         RandPoly_ensure_inv(dd,N,11,9,cc,bb)#n=97
      elif(n==83):
         RandPoly_ensure_inv(dd,N,10,8,cc,bb)#n=83
   fuzhi(gg,poly_Fp, cc)
   fuzhi(hh,poly_Fq, bb)
...
if __name__ == '__main__':
   ...
   while(success!=20):
      ...
      arrf=[0,3,11,16, 25, 32, 37, 45, 48, 51]##randomize
      
      #arrf=[0, 3, 7 , 11, 16, 25, 32, 37, 45, 48, 51, 65, 67, 71, 85, 91]
      #print(len(arrf))
      #当n取小一点的值时，要考虑arrf超过范围
      f_norm=0
      if(n==107):
         #n=107
         RandPoly(ff,N,15,14)
      ...
      H_reduced=[[0 for i in range(N)] for j in range(len(position))] 
      for m in range(0,len(position)):
         for i in range(0,N):
            H_reduced[m][i]=zhuanzhi_H[position[m]][i]
      #print(H_reduced)
      ...
      LambdaA=block_matrix(2,1,[Anull.lift(),q*identity_matrix(n)])#
      #print(LambdaA)
